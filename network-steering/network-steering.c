...
#define REQUIRED_STACK_PROFILE 0
...
// key to use
#define EXTENDED_PAN_ID { 0x8B, 0x12, 0x33, 0x08, 0x1B, 0x0E, 0x54, 0xD2 }
static bool gFilterByExtendedPanId = false;
static uint8_t gExtendedPanIdToFilterOn[8] = EXTENDED_PAN_ID;
static bool gUseConfiguredKey = true;
                                 ...
                                 int8_t rssi)
{
  ...
  if (!(networkFound->allowingJoin
	...
	emberAfCorePrintln("Stack profile mismatch");
    emberAfCorePrintln("%d", networkFound->stackProfile);
    emberAfCorePrintln("%d", networkFound->channel);
    emberAfCorePrintln("0x%2X", networkFound->panId);
    return;
  }
  emberAfCorePrintln("%d", networkFound->stackProfile);
  emberAfCorePrintln("%p nwk found ch: %d, panID 0x%2X, xpan: ",
             ...
             networkFound->panId);
  debugPrintln(emberAfPrintBigEndianEui64(networkFound->extendedPanId));
